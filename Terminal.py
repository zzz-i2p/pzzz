...
class TerminalCommand():
    def get_path(self, paths):
        if paths:
            return paths[0]
        # DEV: On ST3, всегда активный вид.
        #   Убедитесь, что это файл с путь (не временный вид)
    ...
    def run_terminal(self, dir_, parameters):
        try:
            ...
            # Запустить наш процесс
            subprocess.Popen(args, cwd=cwd, env=env)

        except (OSError) as exception:
            print(str(exception))
            sublime.error_message('Терминал: Терминал ' +
                TerminalSelector.get() + ' не найдено')
        except (Exception) as exception:
            sublime.error_message('Терминал: ' + str(exception))
...
class OpenTerminalProjectFolderCommand(sublime_plugin.WindowCommand,
    ...
    def run(self, paths=[], parameters=None):
        ...
        # DEV: Мы требуем сепаратор для присоединения с `/hello` и `/hello-world`
        #   оба матча файла в `/hello` без него
        #   Дополнительные сведения см. В разделе https://github.com/wbond/sublime_terminal/issues/86
